...
def multiextractRoot(splited):
    ...
    if os.path.getsize(splited[1])==0.:
        return
    if "higgs.root" in os.listdir(splited[0]):
        if os.path.getsize(splited[0]+"/higgs.root") >os.path.getsize(splited[1])*1.2:
            return
    print os.listdir(splited[0])
    subprocess.call("tar -xzf outfile.tar.gz",shell=True)
...
def extractRootFiles(options):
    ...
    for i in glob.glob(options.inputFolder+"/*/*/"):
        if len(glob.glob(i+"outfile.tar.gz"))==0:
            log.warning("There is some thing worng in %s no outfile.tar.gz"%(i))
